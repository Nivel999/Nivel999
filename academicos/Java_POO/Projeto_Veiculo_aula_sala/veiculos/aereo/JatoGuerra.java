...
public class JatoGuerra extends Aereo{
    ...
    @Override
    public void mover(){
        System.out.println(String.format("O Jato %s está se movendo com a velocidade %.2f km/h a uma altitude de %.2f pes.", nome, velocidade, altitude));
    }
    
    @Override
    public void ganharAltitude(double incremento){
        if(incremento > 0) altitude = incremento * 3.281;
    }

    @Override
    public void perderAltitude(double decremento){
        if (decremento > 0){
            altitude -= decremento * 3.281;
